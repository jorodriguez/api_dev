...
BEGIN  
		...
		delete from co_asistencia_usuario where fecha <= (getDate('') - 30); 
		...
		delete from co_pago_cargo_balance_alumno where to_char(fecha,'YYYY') <>  '2021';

		raise notice ' == Eliminando pagos, solo dejar los ultimo % días',ANIO_CURSO;
		delete from co_pago_balance_alumno where to_char(fecha,'YYYY') <>  '2021';
		
		raise notice ' == Eliminando cargos ';
		delete from co_cargo_balance_alumno where to_char(fecha,'YYYY') <>  '2021';	
