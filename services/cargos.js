...
const getCatalogoCargos = async(request, response) => {
    console.log("@getCatalogoCargos");
    try {

        const id_sucursal = request.params.id_sucursal;

        const results = await cargoService.getCatalogoCargos(id_sucursal)

        response.status(200).json(results);

    } catch (e) {
        console.log("ERROR al cargar el catalogo de cargos" + e);
        handle.callbackErrorNoControlado(e, response);
    }
};
